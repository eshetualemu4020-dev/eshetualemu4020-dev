<!-- ===================== HEADER ===================== -->

<div align="center">

# 👋 Hi, I'm Eshetu Alemu

### 💻 Computer Science Student | Software Developer | Data Science Enthusiast

<p>
  <img src="https://img.shields.io/badge/Computer%20Science-Student-1F4E79?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Software-Development-2E7D32?style=for-the-badge" />
  <img src="https://img.shields.io/badge/Data-Science-6A1B9A?style=for-the-badge" />
</p>

<p>
  <i>Building practical solutions • Learning continuously • Growing through code</i>
</p>

</div>

---

<!-- ===================== ABOUT ME ===================== -->

## 👨‍💻 About Me

I'm a **Bachelor degree Computer Science student at Hawassa University** passionate about
**software development, web technologies, database systems, and data science**.

I enjoy turning ideas into **practical software solutions**, solving challenging
programming problems, and building applications that address real-world needs.
My technical interests include **Java, Python, JavaScript, PHP, HTML, CSS, SQL,
MySQL, and data analysis**.

I'm committed to **continuous learning and improvement**, with a strong interest
in developing my skills in **software engineering, full-stack development,
database systems, and data science**. I enjoy working on **academic and personal
projects**, collaborating with others, and exploring new technologies.

---

<!-- ===================== CURRENT FOCUS ===================== -->

## 🚀 What I'm Currently Working On

- 💻 Building software and web applications
- 🌐 Improving my front-end and back-end development skills
- 📊 Strengthening my data analysis and data science skills
- ☕ Improving my Java and object-oriented programming skills
- 🐍 Expanding my Python programming skills
- 🔧 Using Git and GitHub for project management and version control
- 📚 Working on academic and personal software projects

---

<!-- ===================== TECH STACK ===================== -->

## 🛠️ Tech Stack

### 💻 Programming Languages

<p align="left">

<a href="https://www.java.com/">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/java/java-original.svg"
width="50" height="50" alt="Java"/>
</a>

<a href="https://www.python.org/">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg"
width="50" height="50" alt="Python"/>
</a>

<a href="https://developer.mozilla.org/en-US/docs/Web/JavaScript">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg"
width="50" height="50" alt="JavaScript"/>
</a>

<a href="https://www.php.net/">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/php/php-original.svg"
width="50" height="50" alt="PHP"/>
</a>

</p>

**Java • Python • JavaScript • PHP**

---

### 🌐 Web Development

<p align="left">

<a href="https://developer.mozilla.org/en-US/docs/Web/HTML">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg"
width="50" height="50" alt="HTML5"/>
</a>

<a href="https://developer.mozilla.org/en-US/docs/Web/CSS">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg"
width="50" height="50" alt="CSS3"/>
</a>

<a href="https://getbootstrap.com/">
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-original.svg"
width="50" height="50" alt="Bootstrap"/>
</a>

</p>

**HTML5 • CSS3 • JavaScript • Bootstrap • PHP**

---

### 🗄️ Database & SQL

<p align="left">

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mysql/mysql-original.svg"
width="50" height="50" alt="MySQL"/>

</p>

**SQL • MySQL • Database Design • Database Management**

---

### 📊 Data Science & Analytics

<p align="left">

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg"
width="50" height="50" alt="Python"/>

</p>

- 🐍 Python
- 📊 Data Analysis
- 📈 Data Visualization
- 🔢 Data Processing
- 🧠 Data Science

---

### 🔧 Tools & Development Environment

<p align="left">
  <a href="https://git-scm.com/">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="50" height="50" alt="Git"/>
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://github.com/">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="50" height="50" alt="GitHub"/>
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://code.visualstudio.com/">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" width="50" height="50" alt="Visual Studio Code"/>
  </a>
  &nbsp;&nbsp;&nbsp;
  <a href="https://www.jetbrains.com/idea/">
    <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/intellij/intellij-original.svg" width="50" height="50" alt="IntelliJ IDEA"/>
  </a>
</p>

**Git • GitHub • Visual Studio Code • antigravity • IntelliJ IDEA**|

---

<!-- ===================== PROJECTS ===================== -->

## 📂 Featured Projects

### 💻 Software Development

I'm interested in building practical applications that combine
**software logic, user interfaces, and databases**.

**Areas include:**

- 🖥️ Java applications
- 🎨 JavaFX desktop applications
- 🌐 Web applications
- 🗄️ Database-driven systems
- 🔐 Application functionality and data management

---
